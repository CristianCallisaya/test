<template>
  <div>
        ...
        <template v-slot:default>
          ...
          <tbody>
            <tr v-for="(solicitud, index) in solicitudes" :key="index">
              ...
              <td class="text-center">
             <v-icon v-show="solicitud.nrosolicitud !== 0" color="green" @click="obtenerDatos(solicitud)">print</v-icon>
                <v-icon v-show="solicitud.nrosolicitud == 0" color="red" @click="correlativoSolicitud(solicitud)">error</v-icon>
              </td>
              <td>
    ...
    <v-dialog v-model="dialogCreate" max-width="500" persistent>
      <v-card>
        <v-form v-on:submit.prevent="createSolicitud" ref="form"
          v-model="valid"
          lazy-validation>
    ...
    <v-dialog v-model="dialogUpdate" max-width="500" persistent>
      <v-card>
        <v-form v-on:submit.prevent="updateSolicitud" ref="form"
          v-model="valid"
          lazy-validation>
...
export default {
  ...
  methods: {
    loadCombox: async function(){
        const response = await solicitudService.filtroSolicitudes(this.comboData,this.buscador );
    ...
    loadEstudiantes: async function () {
      try {
        const response = await estudianteService.getEstudianteSolicitud(this.carreraSelect);
    ...
    loadSolicitudes: async function () {
      try {
        const token = JSON.parse(localStorage.getItem('larave-vue-spa-token'));
        console.log(token)
        const response = await solicitudService.loadSolicitudes(this.comboData,this.pagination.current,token);
        this.solicitudes = response.data.data;
    ...
    updateSolicitud: async function () {
      ...
      try {
        const response = await solicitudService.updateSolicitud(this.editSolicitudData.id, formData);
        this.solicitudes.unshift(response.data);
        this.loadSolicitudes();
    ...
    correlativo: async function(solicitud){
   alert(`as ${solicitud.id}`)
   //   await solicitudService.generarCorrelativoSolicitud(soli.id);
      this.loadSolicitudes();
      this.correlativoData = { ...soli };
    //  this.$store.dispatch("GET_ACTUAL_CORRELATIVO", this.correlativoData);
