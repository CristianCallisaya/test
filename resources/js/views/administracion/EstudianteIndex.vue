          ...
          <tbody>
            <tr v-for="(estudiante, index) in estudiantes" :key="index">
              ...
              <td>
                <span
                  v-bind:style="
                    estudiante.esEstado == 1 ? 'background-color:#5AEE4E' : ''
                  "
    ...
    <v-dialog v-model="dialogCreate" max-width="500" persistent>
      <v-card>
        <v-form v-on:submit.prevent="createEstudiante" ref="form"
          v-model="valid"
          lazy-validation >
          <v-card-title class="blue darken-2 white--text">Crear Estudiante</v-card-title>
          <v-card-text>
            <v-container>
              <v-row>
                <v-col cols="12" sm="6">
...
export default {
  ...
  methods: {
    ...
    if( this.$refs.form.validate() === false){
      ...
      try {
        const response = await estudianteService.createEstudiante(formData);
