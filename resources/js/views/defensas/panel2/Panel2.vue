  ...
  <div>
    ...
    <v-dialog v-model="dialogAprobar" max-width="300">
      <v-card class="container">
        ...
        <v-card-actions>
          <v-btn type="submit" :disabled="!valid">Guardar</v-btn> <v-btn @click="hideDialogAprobar">cancelar</v-btn>
        </v-card-actions>
        </v-form>
...
import actions from "../../../store/modules/solicitudes/actions";
...
import Observacion from "../../administracion/Observacion.vue"
  ...
  */
  ...
  methods: {
    ...
    loadPanel2: async function () {
      try {
        const response = await panel1Service.loadDefensasPanel2(this.pagination.current);
    ...
    loadSearch: async function (e) {
      if (e.length > 2) {
        const response = await panel1Service.loadDefensasPanel1Search(`${e}`);
        this.panels1 = response.data.data;
        console.dir(e);
      }
      if (e.length <= 0) {
        const response = await panel1Service.loadDefensasPanel1(this.pagination.current);
        this.panels1 = response.data.data;
        this.pagination.current = response.data.current_page;
        this.pagination.total = response.data.last_page;
      }
    ...
    loadAsignaciones: async function () {
      try {
        const response = await asignacionService.loadAsignaciones();
  ...
  this.$swal({
    ...
    changeToday(fech) {
      const fechaRecibida = fech.fechaProgramacion;
      console.log(fechaRecibida);
      const fechaActual = new Date(fechaRecibida);
      console.log(fechaActual);
      const formatDate = fechaActual.setMinutes(
